import {
  ...
  x as D,
  y as O,
} from './Mouse.ZrlRGzn3.js';
import { t as k } from './gsap.Bi_c5vh2.js';
var A = e(x(), 1),
  j = (e, t, n, r, i) => ((e - t) * (i - r)) / (n - t) + r,
  M = class e {
    constructor(e, t) {
      ((this.scene = e),
        ...
        (this.lastTime = 0),
        (this.everRendered = !1),
        (this.mousePos = new n(0, 0)),
        (this.cursorPos = new n(0, 0)),
        (this.colorScheme = e.theme === `dark` ? 0 : 1),
        (this.particleScale =
          (this.scene.renderer.domElement.width / this.scene.pixelRatio / 2e3) *
          this.scene.particlesScale));
    }
    static async create(t, n) {
      let r = new e(t, n);
      return (r.createPoints(), await r.createPointsFromImage(), r.init(), r);
    }
      ...
      return new Promise((t, n) => {
        ...
        ((r.src = e),
          (r.onload = () => {
            ...
            ((e.width = 500), (e.height = 500));
            let n = e.getContext(`2d`);
            (n.drawImage(r, 0, 0, 500, 500), t(n.getImageData(0, 0, 500, 500)));
          }),
          (r.onerror = n));
      });
    }
    createPoints() {
      let e = new A.default({
        shape: [500, 500],
        minDistance: j(this.scene.density, 0, 300, 10, 2),
        maxDistance: j(this.scene.density, 0, 300, 11, 3),
        tries: 20,
      }).fill();
      ((this.pointsBaseData = e), (this.pointsData = []));
    ...
    async createPointsFromImage() {
      ...
      for (let t = 0; t < this.textures.length; t++) {
        let n = await this.getImageData(this.textures[t]);
        e.push(n);
      }
      this.nearestPointsData = [];
      let t = [];
      for (let n = 0; n < this.textures.length; n++)
        t.push(this.createPointsDistanceDataWorker(e[n], this.pointsBaseData, n));
      let n = await Promise.all(t);
      (n.sort((e, t) => e.index - t.index),
        n.forEach((e) => {
          this.nearestPointsData.push(e.nearestPoints);
        }));
    }
    createPointsDistanceDataWorker(e, t, n) {
      return new Promise((r, i) => {
        ...
        ((s.onmessage = function (e) {
          let { nearestPoints: t, index: n } = e.data;
          (s.terminate(), URL.revokeObjectURL(o), r({ nearestPoints: t, index: n }));
        }),
          (s.onerror = function (e) {
            (console.error(`Worker error:`, e), s.terminate(), URL.revokeObjectURL(o), i(e));
          }),
          s.postMessage({ imageData: e, pointsBase: t, index: n, density: this.scene.density }));
      });
    }
    createDataTexturePosition(e) {
      ...
      let n = new f(t, this.size, this.size, a, b);
      return ((n.needsUpdate = !0), n);
    }
    createRenderTarget() {
      return new h(this.size, this.size, {
        wrapS: E,
        wrapT: E,
        minFilter: s,
        magFilter: s,
    ...
    init() {
      ...
      let e = new p(new l(2, 2), this.simMaterial);
      this.simScene.add(e);
      let t = new u(),
        i = new Float32Array(this.count * 2),
        a = new Float32Array(this.count * 3),
        s = new Float32Array(this.count * 4);
      for (let e = 0; e < this.count; e++) {
        let t = e % this.size,
          n = Math.floor(e / this.size);
        ((i[e * 2] = t / this.size), (i[e * 2 + 1] = n / this.size));
      }
      for (let e = 0; e < this.count; e++)
        ((s[e * 4] = Math.random()),
          (s[e * 4 + 1] = Math.random()),
          (s[e * 4 + 2] = Math.random()),
          (s[e * 4 + 3] = Math.random()));
      (t.setAttribute(`position`, new S(a, 3)),
        t.setAttribute(`uv`, new S(i, 2)),
        t.setAttribute(`seeds`, new S(s, 4)),
        (this.renderMaterial = new r({
          uniforms: {
            uPosition: { value: this.posTex },
            uTime: { value: 0 },
            uColor1: { value: new w(this.scene.colorControls.color1) },
            uColor2: { value: new w(this.scene.colorControls.color2) },
            uColor3: { value: new w(this.scene.colorControls.color3) },
            uAlpha: { value: 1 },
            uIsHovering: { value: 0 },
        ...
        (this.mesh = new D(t, this.renderMaterial)),
        this.mesh.position.set(0, 0, 0),
        this.mesh.scale.set(5, -5, 5),
        this.scene.scene.add(this.mesh));
    }
    resize() {
      ((this.renderMaterial.uniforms.uRez.value = new n(
        this.scene.renderer.domElement.width,
        this.scene.renderer.domElement.height
      )),
  ...
  N = class {
    constructor(e) {
      ((this.loaded = !1),
        ...
        (this.interactive = !1),
        (this.options.background = this.theme === `dark` ? new w(1184535) : new w(16777215)),
        (this.pixelRatio = e.pixelRatio || window.devicePixelRatio),
        (this.particlesScale = e.particlesScale || 0.5),
    ...
    onHoverStart() {
      (k.to(this, { hoverProgress: 1, duration: 0.5, ease: `power3.out` }),
        k.fromTo(
          this,
          { pushProgress: 0 },
    ...
    onHoverEnd() {
      (k.to(this, { hoverProgress: 0, duration: 0.5, ease: `power3.out` }),
        k.fromTo(
          this,
          { pushProgress: 0 },
    ...
    setPointsTextureFromIndex(e) {
      (k.delayedCall(0.1, () => {
        ...
        k.fromTo(this, { pushProgress: 0 }, { pushProgress: 1, duration: 2, ease: `power2.out` }));
    }
    initCamera() {
      ((this.camera = new O(
        40,
    ...
    async initParticles() {
      this.particles = await M.create(this, this.textures);
    }
    initGUI() {
      ((this.gui = new c({ autoPlace: !1 })),
      ...
      (e
        .addColor(this.colorControls, `color1`)
        .name(`Color 1`)
        .onChange((e) => {
          this.particles.renderMaterial.uniforms.uColor1.value.set(new w(e));
        }),
        e
          .addColor(this.colorControls, `color2`)
          .name(`Color 2`)
          .onChange((e) => {
            this.particles.renderMaterial.uniforms.uColor2.value.set(new w(e));
          }),
        e
          .addColor(this.colorControls, `color3`)
          .name(`Color 3`)
          .onChange((e) => {
            this.particles.renderMaterial.uniforms.uColor3.value.set(new w(e));
          }),
        e
          .add(this, `particlesScale`)
          .name(`Particles Scale`)
          .min(0.1)
          .max(4)
          .step(0.01)
          .onChange((e) => {
            this.particlesScale = e;
          }),
        e
          .add(this, `density`)
          .name(`Density`)
          ...
          .step(10)
          .onChange(async (e) => {
            ((this.density = e), this.verbose, this.killParticles(), await this.initParticles());
          }),
        e.open());
    }
    stop() {
    ...
    onLoaded() {
      ((this.loaded = !0),
        this.onLoadedCallback &&
          typeof this.onLoadedCallback == `function` &&
    ...
    preRender() {
      if (
        ...
        (C.cursor,
          (this.mouse.x = (C.cursor.x - e.left) * (C.screenWidth / e.width)),
          (this.mouse.y = (C.cursor.y - e.top) * (C.screenHeight / e.height)),
          (this.mouse.x = (this.mouse.x / C.screenWidth) * 2 - 1),
          (this.mouse.y = -(this.mouse.y / C.screenHeight) * 2 + 1),
      ...
      if (((this.skipFrame = !this.skipFrame), !this.skipFrame && this.raycastPlane)) {
        ...
        e.length > 0 && this.mouseIsOver
          ? (this.intersectionPoint.copy(e[0].point), (this.isIntersecting = !0))
          : (this.isIntersecting = !1);
      }
    }
    render() {
...
var initMorphParticles = window.initMorphParticles = () => {
  document.querySelectorAll(`[data-morphing-particles-component]`).forEach((e) => {
    let t = e.querySelector(`[data-container]`);
    if (!t) return;
    let n = e.getAttribute(`data-theme`) || `dark`,
      r = parseInt(e.getAttribute(`data-density`) || `100`),
      i = new N({
        container: t,
        theme: n,
        particlesScale: parseFloat(e.getAttribute(`data-particles-scale`) || `1`),
        cameraZoom: parseFloat(e.getAttribute(`data-camera-zoom`) || `3.5`),
        density: r,
        textures: JSON.parse(e.getAttribute(`data-textures`) || `[]`),
        color1: e.getAttribute(`data-color1`) || void 0,
        color2: e.getAttribute(`data-color2`) || void 0,
        color3: e.getAttribute(`data-color3`) || void 0,
        interactive: !0,
        gui: new URLSearchParams(window.location.search).get(`gui`) === `true`,
        verbose: !1,
        onLoaded: () => {
          e.dispatchEvent(new CustomEvent(`loaded`));
        },
      }),
      a = !1,
      o = null,
      s = new IntersectionObserver(
        (e) => {
          e.forEach((e) => {
            ((a = e.isIntersecting), e.isIntersecting ? i.resume() : i.stop());
          });
        },
    ...
    s.observe(t);
    let c = () => {
      ((o = requestAnimationFrame(c)), a && i.render());
    };
    (c(),
      (e.helper = {
        onHover: () => i.onHoverStart(),
        onLeave: () => i.onHoverEnd(),
        setPointsTextureFromIndex: (e) => i.setPointsTextureFromIndex(e),
        kill: () => i.kill(),
      }),
      window.addEventListener(`beforeunload`, () => {
        (s.disconnect(), o !== null && cancelAnimationFrame(o), i.kill());
      }));
  });
};
document.readyState === `loading` ? document.addEventListener(`DOMContentLoaded`, initMorphParticles) : initMorphParticles();
