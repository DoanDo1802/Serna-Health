  ...
  M = class {
    ...
    createDataTexturePosition() {
      ...
      for (let t = 0; t < this.count; t++) {
        let n = t * 4;
        ((e[n + 0] = this.pointsData[t * 2 + 0] * (1 / 250)),
          (e[n + 1] = this.pointsData[t * 2 + 1] * (1 / 250)),
          (e[n + 2] = 0),
          (e[n + 3] = 0));
      }
      let t = new f(e, this.size, this.size, a, x);
      return ((t.needsUpdate = !0), t);
    }
    createRenderTarget() {
      return new g(this.size, this.size, {
        wrapS: D,
        wrapT: D,
        minFilter: s,
        magFilter: s,
    ...
    init() {
      ...
      let e = new m(new l(2, 2), this.simMaterial);
      this.simScene.add(e);
      let t = new u(),
      ...
      for (let e = 0; e < this.count; e++)
        ((s[e * 4] = Math.random()),
          ...
          (s[e * 4 + 2] = Math.random()),
          (s[e * 4 + 3] = Math.random()));
      (t.setAttribute(`position`, new C(a, 3)),
        t.setAttribute(`uv`, new C(i, 2)),
        t.setAttribute(`seeds`, new C(s, 4)),
        (this.renderMaterial = new r({
          uniforms: {
            uPosition: { value: this.posTex },
            uTime: { value: 0 },
            uColor1: { value: new T(this.scene.colorControls.color1) },
            uColor2: { value: new T(this.scene.colorControls.color2) },
            uColor3: { value: new T(this.scene.colorControls.color3) },
            uAlpha: { value: 1 },
            uRingPos: { value: new n(0, 0) },
            uRez: {
              value: new n(
        ...
        (this.mesh = new O(t, this.renderMaterial)),
    ...
    update() {
      ...
      let t = (this.noise.getVal(this.scene.time * 0.66 + 94.234) - 0.5) * 2,
        n = (this.noise.getVal(this.scene.time * 0.75 + 21.028) - 0.5) * 2;
      (this.cursorPos.set(t * 0.2, n * 0.1),
        this.scene.isIntersecting
          ? (this.cursorPos.set(
              this.scene.intersectionPoint.x * (this.scene.ringReach || 0.28) + t * 0.1,
              this.scene.intersectionPoint.y * (this.scene.ringReach || 0.28) + n * 0.1
            ),
            this.ringPos.set(
              this.ringPos.x + (this.cursorPos.x - this.ringPos.x) * (this.scene.lerpSpeed || 0.045),
              this.ringPos.y + (this.cursorPos.y - this.ringPos.y) * (this.scene.lerpSpeed || 0.045)
            ))
          : (this.cursorPos.set(t * 0.2, n * 0.1),
            this.ringPos.set(
  ...
  N = class {
    constructor(e) {
      ((this.loaded = !1),
        ...
        (this.options.background = this.theme === `dark` ? new T(0) : new T(16777215)),
        ...
        (this.mouseIsOver = !1),
        (this.raycastPlane = new m(
          new l(12.5, 12.5),
          new p({ color: 16711680, visible: !1, side: 2 })
        )),
    ...
    initScene() {
      ((this.colorControls = {
        ...
        (this.lerpSpeed = this.options.lerpSpeed || 0.045),
        (this.ringReach = this.options.ringReach || 0.28),
        this.initParticles(),
        this.options.gui && this.initGUI(),
        this.onLoaded());
    }
    initParticles() {
      this.particles = new M(this);
    }
    initGUI() {
      ...
      (e
        ...
        .name(`Color 1`)
        .onChange((e) => {
          this.particles.renderMaterial.uniforms.uColor1.value.set(new T(e));
        }),
        e
          .addColor(this.colorControls, `color2`)
          .name(`Color 2`)
          .onChange((e) => {
            this.particles.renderMaterial.uniforms.uColor2.value.set(new T(e));
          }),
        e
          .addColor(this.colorControls, `color3`)
          .name(`Color 3`)
          .onChange((e) => {
            this.particles.renderMaterial.uniforms.uColor3.value.set(new T(e));
          }),
        e
          .add(this, `ringWidth`)
          .name(`Ring Width`)
        ...
        e
          .add(this, `ringDisplacement`)
          .name(`Displacement`)
          .min(0.01)
          .max(1)
          .step(0.01)
          .onChange((e) => {
            this.ringDisplacement = e;
          }),
        e
          .add(this, `density`)
          .name(`Density`)
          ...
          .step(10)
          .onChange((e) => {
            ((this.density = e), this.verbose, this.killParticles(), this.initParticles());
          }),
        e.open());
    }
    stop() {
    ...
    preRender() {
      if (
        ...
        (w.cursor,
          (this.mouse.x = (w.cursor.x - e.left) * (w.screenWidth / e.width)),
          (this.mouse.y = (w.cursor.y - e.top) * (w.screenHeight / e.height)),
          (this.mouse.x = (this.mouse.x / w.screenWidth) * 2 - 1),
          (this.mouse.y = -(this.mouse.y / w.screenHeight) * 2 + 1),
...
var initMainParticles = () => {
  document.querySelectorAll(`[data-main-particles-component]`).forEach((e) => {
    let t = e.querySelector(`[data-container]`);
    if (!t) return;
    if (t.querySelector(`canvas`)) return;
    let n = e.getAttribute(`data-theme`) || `light`,
      r = parseFloat(e.getAttribute(`data-ring-width`) || `0.15`),
      i = parseFloat(e.getAttribute(`data-ring-width2`) || `0.05`),
      a = parseFloat(e.getAttribute(`data-ring-displacement`) || `0.15`),
      o = parseInt(e.getAttribute(`data-density`) || `200`),
      lerpSpeed = parseFloat(e.getAttribute(`data-lerp`) || `0.045`),
      ringReach = parseFloat(e.getAttribute(`data-ring-reach`) || `0.28`),
      s = new N({
        container: t,
        theme: n,
        particlesScale: parseFloat(e.getAttribute(`data-particles-scale`) || `0.75`),
        density: o,
        interactive: !0,
      ...
      u = new IntersectionObserver(
        (e) => {
          e.forEach((e) => {
            ((c = e.isIntersecting), e.isIntersecting ? s.resume() : s.stop());
          });
        },
    ...
    u.observe(t);
    let d = () => {
      ((l = requestAnimationFrame(d)), c && s.render());
    };
    (d(),
      window.addEventListener(`beforeunload`, () => {
        (u.disconnect(), l !== null && cancelAnimationFrame(l), s.kill());
      }));
  });
};
window.initMainParticles = initMainParticles;
document.readyState === `loading` ? document.addEventListener(`DOMContentLoaded`, initMainParticles) : initMainParticles();
