...
const inputBaseClasses =
  'w-full px-3.5 py-2 rounded-xl border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.08] focus:bg-white/[0.08] focus:border-primary focus:ring-1 focus:ring-primary/30 outline-none text-xs sm:text-sm text-content-primary transition-all font-medium placeholder:text-content-muted/60 disabled:bg-white/[0.02] disabled:border-white/[0.06] disabled:text-content-secondary disabled:cursor-not-allowed';
...
function PatientForm({
  ...
  return (
    <form
      ...
      className="bg-surface border border-outline-variant/80 rounded-3xl p-5 sm:p-7 shadow-card h-full flex flex-col justify-between"
    >
      <div>
        {/* Card Header Strip */}
        <div className="pb-4 border-b border-outline-variant/60 mb-6">
          <h2 className="text-base sm:text-lg font-bold tracking-tight text-content-primary m-0">
            {patient
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {!patient && hasOwnProfile && (
            <div className="flex flex-col gap-1.5 sm:col-span-2 p-4 rounded-2xl bg-surface-container/70 border border-outline-variant/60">
              ...
              <div className="relative flex items-center">
                <select
                  ...
                  className="w-full px-3.5 py-2 pr-10 rounded-xl border border-outline-variant/80 bg-surface focus:border-primary outline-none text-xs sm:text-sm text-content-primary transition-all font-medium cursor-pointer appearance-none shadow-xs"
                >
                ...
                <ChevronDown className="absolute right-3.5 w-3.5 h-3.5 text-content-muted pointer-events-none" />
              </div>
          ...
          <div className="flex flex-col gap-1.5 sm:col-span-2">
            ...
            <input
              ...
              className={`${inputBaseClasses} uppercase placeholder:normal-case`}
            />
          </div>
              ...
              {isEditing ? (
                <>
                  <select
                    ...
                    className={`${inputBaseClasses} pr-10 cursor-pointer appearance-none`}
                  >
        ...
        <div className="mt-8 pt-5 border-t border-outline-variant/60">
          <div className="mb-4">
            <h3 className="text-sm sm:text-base font-bold tracking-tight text-content-primary m-0">
      ...
      {/* Action Buttons */}
      <div className="mt-8 pt-5 border-t border-outline-variant/60 flex items-center justify-between gap-3">
        {patient && isDependent ? (
          <button
            type="button"
            onClick={onDeleteRequest}
            className="px-4 py-2 rounded-full border border-rose-500/30 text-rose-500 hover:bg-rose-500/10 text-xs sm:text-sm font-semibold transition-all cursor-pointer flex items-center gap-1.5"
          >
        ...
        <div className="flex items-center gap-2.5">
          {isEditing ? (
            <>
              {patient && (
                <button
                  ...
                  className="px-4 py-2 rounded-full bg-surface-container hover:bg-surface-container-high border border-outline-variant text-xs sm:text-sm font-semibold text-content-secondary hover:text-content-primary transition-all cursor-pointer"
                >
                  Hủy Bỏ
                </button>
              )}
              <button
                type="submit"
                disabled={isSaving}
                className="px-5 py-2 rounded-full bg-primary hover:bg-primary-hover text-white text-xs sm:text-sm font-bold shadow-xs hover:shadow-card transition-all cursor-pointer disabled:opacity-50 flex items-center gap-2"
              >
                {isSaving ? (
            ...
            <button
              ...
              className="px-5 py-2 rounded-full bg-primary hover:bg-primary-hover text-white text-xs sm:text-sm font-bold shadow-xs hover:shadow-card transition-all cursor-pointer flex items-center gap-1.5"
            >
...
export function PatientProfilePage() {
  ...
  return (
    <div className="w-full font-sans text-content-primary selection:bg-primary selection:text-white pt-2 sm:pt-4 pb-8 flex-1 flex flex-col min-h-[calc(100vh-140px)]">
        ...
        <div className="flex items-center gap-2.5">
          <div className="relative">
            <select
              ...
              className="appearance-none bg-surface-container hover:bg-surface-container-high border border-outline-variant/80 hover:border-primary rounded-full px-4 py-1.5 pr-9 text-xs font-semibold text-content-primary focus:outline-none focus:border-primary focus:bg-surface shadow-xs cursor-pointer transition-all"
            >
            ...
            <ChevronDown className="absolute right-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-content-muted pointer-events-none" />
          </div>
        </div>
      </div>

      {isLoading ? (
        <div className="w-full flex flex-col items-center justify-center py-20 bg-surface rounded-3xl border border-outline-variant shadow-card">
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-4 flex flex-col gap-5 h-full justify-between">
            ...
            <div className="bg-surface border border-outline-variant/80 rounded-3xl p-5 sm:p-6 shadow-card flex-1 flex flex-col min-h-0 justify-between">
              <div>
                ...
                <p className="text-xs text-content-secondary mt-2.5 mb-3.5 leading-relaxed">
                ...
                <div className="overflow-y-auto pr-1 flex flex-col gap-2.5 max-h-[380px] lg:max-h-[440px] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-outline-variant [&::-webkit-scrollbar-track]:bg-transparent">
                  {identifiers.length === 0 ? (
                    <div className="py-8 px-4 rounded-2xl bg-surface-container/40 border border-dashed border-outline-variant text-center">
                      <p className="text-xs font-medium text-content-secondary m-0">
                      ...
                      {activePatientId && (
                        <button
                          type="button"
                          onClick={() => setIsAddIdentifierOpen(true)}
                          className="mt-3.5 px-4 py-1.5 rounded-full bg-primary hover:bg-primary-hover text-white text-xs font-semibold cursor-pointer shadow-xs transition-colors"
                        >
                          + Bổ sung ngay
                        </button>
                      )}
                    ...
                    identifiers.map((item) => (
                      <div
                        key={item.id}
                        className="p-3.5 rounded-2xl bg-surface-container/50 border border-outline-variant/60 flex flex-col gap-2 relative overflow-hidden shrink-0 hover:bg-surface-container transition-colors"
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-mono font-bold text-content-primary bg-surface px-2 py-0.5 rounded-md border border-outline-variant shadow-2xs">
                            {item.identifierType}
                          </span>
                          ...
                          {item.verificationSource === 'MANUALLY_VERIFIED' ? (
                            ...
                            <StatusBadge variant="primary">Nhân viên thu thập</StatusBadge>
                          ) : (
                            <StatusBadge variant="warning">Tự khai báo</StatusBadge>
                          )}
                        </div>

                        <div className="flex items-center justify-between pt-0.5">
                          <div className="text-sm sm:text-base font-mono font-bold tracking-widest text-content-primary">
                            •••• •••• {item.displaySuffix || '****'}
                          </div>
                          <span className="text-[11px] text-content-muted font-mono font-semibold">
                            {item.jurisdiction}
                          </span>
                        </div>
              ...
              {/* Security Hint */}
              <div className="mt-3.5 p-3 rounded-2xl bg-surface-container/50 border border-outline-variant/70 flex items-start gap-2.5 text-xs text-content-secondary shrink-0">
            ...
            <div className="bg-surface border border-outline-variant/80 rounded-3xl p-5 sm:p-6 shadow-card shrink-0 flex flex-col gap-1.5">
              <h4 className="text-[11px] font-semibold uppercase tracking-wider text-content-muted m-0">
                Liên Kết Tài Khoản
              </h4>
              <p className="text-xs text-content-secondary m-0 leading-relaxed">
                ...
                nhân. Trùng lặp thông tin nếu có sẽ được bộ phận tiếp nhận kiểm tra thủ công.
              </p>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleAddIdentifier} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-2xl border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.08] focus:bg-white/[0.08] focus:border-primary focus:ring-1 focus:ring-primary/30 outline-none text-xs font-semibold text-content-primary cursor-pointer transition-all"
                >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-2xl border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.08] focus:bg-white/[0.08] focus:border-primary focus:ring-1 focus:ring-primary/30 outline-none text-sm font-mono text-content-primary transition-all"
                />
                <span className="text-[11px] text-content-muted">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-2xl border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.08] focus:bg-white/[0.08] focus:border-primary focus:ring-1 focus:ring-primary/30 outline-none text-xs text-content-primary transition-all"
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-2xl border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.08] focus:bg-white/[0.08] focus:border-primary focus:ring-1 focus:ring-primary/30 outline-none text-xs font-mono text-content-primary uppercase transition-all"
                />
              </div>
