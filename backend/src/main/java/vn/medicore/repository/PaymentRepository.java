...
public interface PaymentRepository {
    ...
    Optional<PaymentRow> paymentByIdForUpdate(UUID id);

    Optional<PaymentRow> paymentByIntentId(UUID paymentIntentId);

    Optional<PaymentRow> paymentByIntentIdForUpdate(UUID paymentIntentId);

    Optional<PaymentRow> paymentByProviderTransactionId(String provider, String providerTransactionId);

    void insertRescheduleTopUp(RescheduleTopUpRow row);

    Optional<RescheduleTopUpRow> rescheduleTopUpByPaymentIntentIdForUpdate(UUID paymentIntentId);

    void updateRescheduleTopUpStatus(UUID id, String status, Instant consumedAt, String expectedStatus);
